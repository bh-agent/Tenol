...
3. **결과 입력 +/- 버튼 32x32px** → **수정 완료** (w-11 h-11 = 44px)
4. **캐러셀 점 인디케이터 6px** → **수정 완료** (p-2 터치 패딩 추가)
5. **로그인 이용약관 링크 비활성** → **수정 완료** (/terms, /privacy 링크 연결)

### High (조기 수정 권장) - 7건
- ~~로그인 페이지 `<img>` → Next.js `<Image>` 전환~~ → **수정 완료**
- ~~대진표 참가자 칩 버튼 18px → 44px~~ → **수정 완료** (p-2 터치 영역)
- ~~피드/클럽 페이지 loading.tsx 추가 필요~~ → **수정 완료** (8개 라우트에 loading.tsx 추가)
- ~~`alert()`/`confirm()` → Toast/Modal 전환~~ → **수정 완료** (18개 파일, 전체 교체)
- ~~에러 바운더리 추가~~ → **수정 완료** (6개 라우트에 error.tsx 추가)
- ~~드롭다운 메뉴 aria 속성 추가~~ → **수정 완료** (3개 드롭다운에 role/aria 속성)
- ~~설정 페이지 로드 실패 시 빈 폼 저장 방지~~ → **수정 완료** (loadFailed 가드)
...
### 개선 권장 (nice-to-have 5건)
- 성별 fallback 시 사용자에게 경고 표시
- 대진표 검증 경고를 UI에 표시
- 매치 상태 전환 검증 (completed → upcoming 방지)
- MVP 계산을 승률 기반으로 변경
- ~~게스트 모집에 게임 포맷 선택지 추가~~ → **수정 완료** (게스트 모집 폼에 복식/단식/혼복 선택지 추가)
- ~~"혼합 복식" → "혼복" 용어 통일~~ → **수정 완료** (전체 앱에서 통일)
- ~~역전승 업적 (set_scores 의존)~~ → **제거 완료** (set_scores 미사용으로 clutch 업적 삭제)
...
### 즉시 (이번 주)
- [x] 이용약관/개인정보 처리방침 페이지 생성 및 링크 연결 → **완료** (/terms, /privacy 페이지 + PIPA 준수)
- [x] 결과 입력 버튼 터치 영역 확대 (44px) → **완료**
- [x] alert()/confirm() → Toast/Modal 전환 → **완료** (18개 파일)
- [x] loading.tsx 추가 (feed, clubs/[clubId]) → **완료** (8개 라우트)

### 단기 (2주 내)
- [ ] 웹 푸시 알림 (Service Worker)
- [x] 유저 검색 페이지 (/search) → **완료** (검색 API + 필터 + 하단 네비 탭 추가)
- [x] 일반 멤버 초대 링크 공유 허용 → **완료**
- [ ] 카카오 비즈앱 전환 → 이메일 동의항목 활성화 (카카오 개발자 콘솔에서 설정 필요)

### 중기 (1~2개월)
- [ ] 반복 경기 템플릿
- [ ] 세트 기반 점수 입력
- [x] 공유 가능한 스탯 카드 → **완료** (프로필 "내 전적 공유" 버튼, 1080x1080 이미지 생성)
- [ ] 경기별 채팅
- [x] 대기자 명단 → **완료** (DB 마이그레이션 + 자동 승격 + UI)
